...
proc_freepagetable(pagetable_t pagetable, uint64 sz)
{
  ...
  pte_t *pte= walk(pagetable, SHM_ADDR, 0);
if(pte && (*pte & PTE_V))
uvmunmap(pagetable, SHM_ADDR, 1,0);
...
if(p->shm_valid){
pte_t *pte = walk(p->pagetable, SHM_ADDR,0);
...
if(mappages(np->pagetable, SHM_ADDR,PGSIZE,pa,PTE_R | PTE_W | PTE_U)<0){
freeproc(np);
release(&np->lock);
return -1;
}
...
uint wait_time;
int response_ratio;
wait_time=now - p->ctime;
if(p->burst_time<=0)
p->burst_time=1;
response_ratio=((wait_time + p->burst_time) *100)/p->burst_time;
if(best==0){
best=p;
}
else{
uint best_wait;
int best_response_ratio;
best_wait=now - best->ctime;
best_response_ratio = ((best_wait+ best->burst_time)*100)/best->burst_time;
if(response_ratio > best_response_ratio) {
release(&best->lock);
best=p;
}
else {
...
if(mappages(p->pagetable, SHM_ADDR,PGSIZE,pa,PTE_R | PTE_W | PTE_U)<0){
kfree((void*)pa);
return 0;
}
